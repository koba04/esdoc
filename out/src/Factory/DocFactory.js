  ...
  _createClass(DocFactory, [{
    ...
    value: function push(node, parentNode) {
      ...
      if (node.trailingComments && isLastNodeInParent) {
        ...
        var _results4 = this._traverseComments(parentNode, null, node.trailingComments);
        (_results3 = this._results).push.apply(_results3, _toConsumableArray(_results4));
      }
    }
  }, {
    ...
    value: function _traverseComments(parentNode, node, comments) {
      ...
      if (node.type === 'MethodDefinition' && node.key.type === 'Literal') {
        ...
        try {
          for (var _iterator = (this._ast.comments || [])[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
